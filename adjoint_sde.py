...
import torch
import sde_core as baseFunc
import sde_core as misc
from sde_core import TensorOrTensors


class AdjointSDE(baseFunc.BaseSDE):

    def __init__(self,
                 forward_sde: baseFunc.ForwardSDE,
        ...
        self.f = {
            'ito': {
                'diagonal': self.f_corrected_diagonal,
                'scalar': self.f_corrected_default,
            }.get(forward_sde.noise_type),
            'stratonovich': self.f_uncorrected
        }.get(forward_sde.sde_type)
        self.f_and_g_prod = {
            'ito': {
                'diagonal': self.f_and_g_prod_corrected_diagonal,
                'scalar': self.f_and_g_prod_corrected_default,
            }.get(forward_sde.noise_type),
            'stratonovich': self.f_and_g_prod_uncorrected
        }.get(forward_sde.sde_type)
