...
"""Virtual Brownian motion: interval increments, reversed paths, path/tree wrappers."""
...
def _davie_foster_approximation(W, H, h, levy_area_approximation, get_noise):
    if levy_area_approximation in ('none', 'space-time'):
        return None
        ...
        if levy_area_approximation == 'foster':
...
class BrownianInterval(BaseBrownian, _Interval):
    ...
    def __init__(self,
        ...
        if levy_area_approximation not in ['none','space-time','davie','foster']:
            raise ValueError(f"`levy_area_approximation` must be one of {['none','space-time','davie','foster']}, but got "
        ...
        self._have_H = self._levy_area_approximation in ('space-time',
                                                         'davie',
                                                         'foster')
        self._have_A = self._levy_area_approximation in ('davie',
                                                         'foster')
...
class ReverseBrownian(BaseBrownian):
    """Query a forward Brownian on reversed times ``(-tb, -ta)`` for adjoint steps."""
...
class BrownianPath(BaseBrownian):
    ...
    def __init__(self, t0: Scalar, w0: Tensor, window_size: int = 8):
        ...
        t1 = t0 + 1
        self._w0 = w0
        self._interval = BrownianInterval(t0=t0, t1=t1, size=w0.shape, dtype=w0.dtype,
...
class BrownianTree(BaseBrownian):
...
def brownian_interval_like(y: Tensor,
    ...
    return BrownianInterval(t0=t0, t1=t1, size=size, dtype=dtype, device=device, **kwargs)
